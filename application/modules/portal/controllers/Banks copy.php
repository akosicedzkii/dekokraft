...
class Banks extends CI_Controller {
    public function __construct()
    {
        ...
        $this->load->model("portal/banks_model"); 
	...
	{
        $this->banks_model->name = $this->input->post("name");
        ...
        $this->banks_model->created_by =  $this->session->userdata("USERID");
        echo $this->banks_model->insert_banks();
	}

	public function edit_banks()
	{
        $banks_id = $this->input->post("id");
        $this->banks_model->name = $this->input->post("name");
        $this->banks_model->code = $this->input->post("code");
        $this->banks_model->address = $this->input->post("address");
        $this->banks_model->bank_details = $this->input->post("bank_details");
	...
	public function delete_banks()
	{
        ...
        $this->db->where("id",$id);
        $data_banks = $this->db->get("banks");
        $this->db->where("id",$id);
        $data["status"] = 3;
        echo $result = $this->db->update("banks",$data);
        $data = json_encode($data_banks->row());
        $this->logs->log = "Deleted banks - ID:". $data_banks->row()->id .", banks Title: ".$data_banks->row()->name ;
    ...
    public function get_banks_selection()
    {
        ...
        foreach ($rResult->result_array() as $aRow) {
            ...
            foreach ($select_columns as $col) {
                    ...
                    else if($col == "cover_image")
                    {
                        if($aRow[$col] != null)
                        {    
                            $row[] = "<a href=\"#\" onclick='return false;'><img class='img-thumbnail' src='".base_url()."uploads/banks/".$aRow[$col]."' style='height:70px;' onclick='img_preview(\"".$aRow[$col]."\");return false;'></a>";
                        }
                        else
                        {
                            $row[] = "None";
