...
class Product_variants extends CI_Controller {
    ...
    public function get_product_variants_list()
    {
        ...
        $this->dt_model->where  = array("t1.id","t4.class","t4.code","t1.description","t1.location","t1.color","t1.proto","t1.molds","t1.status");  
        if($this->session->userdata("USERTYPE") ==1)
        {
            $this->dt_model->where  = array("t1.id","t4.class","t4.code","t1.description","t1.location","t1.color","t1.proto","t1.molds","t1.status","t1.date_created","t2.username","t1.date_modified","t3.username");  
        }
