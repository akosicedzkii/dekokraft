...
class Products extends CI_Controller {
    ...
    public function get_products_list()
    {
        ...
        $this->dt_model->select_columns = array("t1.id","t1.class","t1.code","t1.description","t1.inner_carton","t1.master_carton","t1.weight_of_box","t1.minimum_of_quantity","t1.lowest_cost","t1.fob","(SELECT price from product_price_history where product_id=t1.id order by id desc limit 1,1) as old_price","t1.status","(SELECT COUNT(*) FROM STOCKS WHERE status = 0 and product_variant_id in(SELECT id from product_variants WHERE product_id = t1.id   and product_variants.status = 1)) as quantity");
        if($this->session->userdata("USERTYPE") ==1)
        {
            $this->dt_model->select_columns = array("t1.id","t1.class","t1.code","t1.description","t1.inner_carton","t1.master_carton","t1.weight_of_box","t1.minimum_of_quantity","t1.lowest_cost","t1.fob","(SELECT price from product_price_history where product_id=t1.id order by id desc limit 1,1) as old_price","t1.status","(SELECT COUNT(*) FROM STOCKS WHERE status = 0 and  product_variant_id in(SELECT id from product_variants WHERE product_id = t1.id   and product_variants.status = 1)) as quantity","t1.date_created","t2.username as created_by","t1.date_modified","t3.username as modified_by");
        }
