...
<div class="modal fade" id="blogsModal" role="dialog"  data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             ...
             <input type="hidden" id="blogsID">
            </div>
            <div class="modal-body">
                <div>
                    <form class="form-horizontal" id="blogsForm" data-toggle="validator">
                        <div class="box-body">
                            <div class="form-group">
                                <label for="inputBlogsTitle" class="col-sm-2 control-label">Title</label>

                                <div class="col-sm-10">
                                <input type="text" class="form-control" id="inputBlogsTitle" placeholder="Title" required>
                            ...
                            <div class="form-group">
                                <label for="inputBlogsAuthor" class="col-sm-2 control-label">Author</label>

                                <div class="col-sm-10">
                                <input type="text" class="form-control" id="inputBlogsAuthor" placeholder="Author" required>
                                <div class="help-block with-errors"></div>
                                </div>
                            </div>
                            <div class="form-group">
            ...
            <button type="button" class="btn btn-primary" id="saveBlogs">Save Blogs</button>
            </div>
        </div>
    <!-- /.modal-content -->
    </div>
...
<!-- /.modal -->
<div class="modal fade" id="deleteBlogsModal"  role="dialog"  data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             ...
             <h3 class="modal-title">Delete Blogs</h3>
            </div>
            <div class="modal-body">
            ...
            <div class="modal-footer">
            <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
            <button type="button" class="btn btn-danger" id="deleteBlogs">Delete</button>
            </div>
        </div>
    <!-- /.modal-content -->
    </div>
    ...
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             ...
             <h3 class="modal-title">Blogs Cover Image Gallery</h3>
            </div>
            <div class="modal-body">
...
<script>
    ...
    var main = function(){
        var table = $('#blogsList').DataTable({
            ...
            "processing" : true,
            "serverSide" : true, 
            "ajax" : "<?php echo base_url()."portal/blogs/get_blogs_list";?>",
        ...
        $("#addBtn").click(function(){
            $("#blogsModal .modal-title").html("Add <?php echo ucfirst($module_name);?>");
            $("#action").val("add");
        ...
        var image_error = "";
        $("#blogsForm").validator().on('submit', function (e) {
            var btn = $("#saveBlogs");
            ...
            if (e.isDefaultPrevented()) {
                ...
                e.preventDefault();
                var title = $("#inputBlogsTitle").val();
                var description = $("#inputDescription").val();
                var content = editor.getData();;
                var status = $("#inputStatus").val();
                var blogs_id = $("#blogsID").val();

                var formData = new FormData();
                formData.append('id', blogs_id);
                formData.append('title', title);
                formData.append('description', description);
                formData.append('content', content);
                formData.append('author', $("#inputBlogsAuthor").val());
                ...
                var url = "<?php echo base_url()."portal/blogs/add_blogs";?>";
                var message = "New blogs successfully added";
                if(action == "edit")
                {
                    url =  "<?php echo base_url()."portal/blogs/edit_blogs";?>";
        ...
        $("#deleteBlogs").click(function(){
            ...
            $.ajax({
                        data: data,
                        type: "post",
                        url: "<?php echo base_url()."portal/blogs/delete_blogs";?>",
                        success: function(data){
                            //alert("Data Save: " + data);
                            btn.button("reset");
                            table.draw("page");
                            $("#deleteBlogsModal").modal("hide");
        ...
        $('#blogsModal').on('hidden.bs.modal', function (e) {
            $(this)
    ...
    function _edit(id)
    {
        $("#blogsModal .modal-title").html("Edit <?php echo ucfirst($module_name);?>");
        $(".add").hide();    
        $('#blogsForm').validator();    
        $("#action").val("edit");
        $("#inputCoverImage").removeAttr("required");
        $("#inputBlogsname").attr("data-remote","<?php echo base_url()."blogs/check_blogsname_exist?method=edit&blogs_id=";?>" + id);
        var data = { "id" : id }
        $.ajax({
                data: data,
                type: "post",
                url: "<?php echo base_url()."portal/blogs/get_blogs_data";?>",
                success: function(data){
                    data = JSON.parse(data);
                    $("#inputBlogsTitle").val(data.blogs.title);
                    $("#inputDescription").val(data.blogs.description);
                    editor.setData(data.blogs.content);
                    $("#inputStatus").val(data.blogs.status).trigger('change');
                    $('#inputCoverImage').val(data.blogs.cover_image_id);
                    $('#inputBlogsAuthor').val(data.blogs.author);
                    $('#coverImgPrev').attr("src","<?php echo base_url()."/uploads/blogs/"; ?>" + data.blogs.cover_image);
                    $("#blogsID").val(data.blogs.id);
                    $("#blogsModal").modal("show");
                },
                error: function (request, status, error) {
                    alert(request.responseText);
    ...
    {
        $("#deleteBlogsModal .modal-title").html("Delete <?php echo rtrim(ucfirst($module_name),"s");?>");
    ...
    {
        $("#imgPreview").attr("src","<?php echo base_url()."uploads/blogs/"?>"+img_src);
    ...
    var new_table = $('#imageGalleryTable').DataTable({  
            ...
            "ajax" : "<?php echo base_url()."portal/media/get_media_list?module=blogs";?>",
            "initComplete": function(settings,json){
                $('[data-toggle="tooltip"]').tooltip()
            }
            ,"columnDefs": [
