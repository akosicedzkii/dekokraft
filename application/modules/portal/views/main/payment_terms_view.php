...
<div class="box" id="main-list">
    ...
    <!-- /.box-header -->
    <div class="box-body">
        <table id="payment_termsList" class="table table-bordered table-striped">
        <thead>
        <tr>
            <th>ID</th>
            <th>Name</th>
...
<div class="modal fade" id="payment_termsModal" role="dialog"  data-backdrop="static">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
             ...
             <h3 class="modal-title">Add Payment Terms</h3>
             <input type="hidden" id="action">
             <input type="hidden" id="payment_termsID">
            </div>
            <div class="modal-body">
                <div>
                    <form class="form-horizontal" id="payment_termsForm" data-toggle="validator">
                        <div class="box-body">
            ...
            <button type="button" class="btn btn-primary" id="savePayment_terms">Save <?php echo $module_name;?></button>
            </div>
        </div>
    <!-- /.modal-content -->
    </div>
...
<!-- /.modal -->
<div class="modal fade" id="deletePayment_termsModal"  role="dialog"  data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
            <button type="button" class="btn btn-default pull-left" data-dismiss="modal">Close</button>
            <button type="button" class="btn btn-danger" id="deletePayment_terms">Delete</button>
            </div>
        </div>
    <!-- /.modal-content -->
    </div>
...
<script>
    ...
    var main = function(){
        var table = $('#payment_termsList').DataTable({
            "language": {                
                "infoFiltered": ""
            },  
            'autoWidth'   : true,
            "processing" : true,
            "serverSide" : true, 
            "ajax" : "<?php echo base_url()."portal/payment_terms/get_payment_terms_list";?>",
        ...
        $("#addBtn").click(function(){
            $("#payment_termsModal .modal-title").html("Add <?php echo ucfirst($module_name);?>");
            $("#action").val("add");
            $("#inputCoverImage").attr("required","required");
            $('#payment_termsForm').validator();
            $("#payment_termsModal").modal("show");
        });

        $("#savePayment_terms").click(function(){
            $("#payment_termsForm").submit();
        });

        var image_correct = true;
        var image_error = "";
        $("#payment_termsForm").validator().on('submit', function (e) {
           
            var btn = $("#savePayment_terms");
            ...
            if (e.isDefaultPrevented()) {
                ...
                // Attach file
                 //fromthis    
                 var url = "<?php echo base_url()."portal/payment_terms/add_payment_terms";?>";
                var message = "New payment_terms successfully added";
                if(action == "edit")
                {
                    url =  "<?php echo base_url()."portal/payment_terms/edit_payment_terms";?>";
                    message = "Payment_terms successfully updated";
                }
                ...
                $.ajax({
                    ...
                    else
                    {
                         ...
                         $("#payment_termsForm").validator('destroy');
                         $("#payment_termsModal").modal("hide"); 
                         $('#uploadBoxMain').html('');          
                    }
                });
            }
        ...
        $("#deletePayment_terms").click(function(){
            var btn = $(this);
            ...
            $.ajax({
                        data: data,
                        type: "post",
                        url: "<?php echo base_url()."portal/payment_terms/delete_payment_terms";?>",
                        success: function(data){
                            //alert("Data Save: " + data);
                            btn.button("reset");
                            table.draw("page");
                            $("#deletePayment_termsModal").modal("hide");
                            toastr.error('Payment_terms ' + deleteItem + ' successfully deleted');
                        },
        ...
        $('#payment_termsModal').on('hidden.bs.modal', function (e) {
            $(this)
    ...
    function _edit(id)
    {
        $("#payment_termsModal .modal-title").html("Edit <?php echo ucfirst($module_name);?>");
        $(".add").hide();    
        $('#payment_termsForm').validator();    
        $("#action").val("edit");
        ...
        $.ajax({
                data: data,
                type: "post",
                url: "<?php echo base_url()."portal/payment_terms/get_payment_terms_data";?>",
                success: function(data){
                    data = JSON.parse(data);
                    $("#name").val(data.payment_terms.name);
    ...
    {
        $("#deletePayment_termsModal .modal-title").html("Delete Payment_terms");
        $("#deleteItem").html(item);
        $("#deleteKey").val(id);
        $("#deletePayment_termsModal").modal("show");
    }
    
    function img_preview(img_src)
    {
        $("#imgPreview").attr("src","<?php echo base_url()."uploads/payment_terms/"?>"+img_src);
    ...
    var new_table = $('#imageGalleryTable').DataTable({  
            ...
            "ajax" : "<?php echo base_url()."portal/media/get_media_list?module=payment_terms";?>",
            "initComplete": function(settings,json){
                $('[data-toggle="tooltip"]').tooltip()
            }
            ,"columnDefs": [
